const fs = require('fs');
let code = fs.readFileSync('src/components/onboarding/options.ts', 'utf8');

// The file currently has duplicate declarations because the previous tool appended new ones at the bottom instead of replacing the old ones.
// I will strip out the first ones if there are duplicates.

// Actually, it's easier to just overwrite the entire file with a clean list.

const newCode = `// Centralized option lists for the marketing-agency onboarding flow.
...
export const PRIOR_GOV_EXPERIENCE = [
  { value: 'yes', label: 'Yes' },
  { value: 'no', label: 'No' },
  { value: 'limited', label: 'Limited subcontracting experience' },
];

export const PROPOSAL_READINESS = [
...
fs.writeFileSync('src/components/onboarding/options.ts', newCode);
